...
def hard_constraint_check(parent_row, garden_row):
    ...
    # Religious orientation is kept as a strong preference, not a hard constraint by default.
    # This prevents the system from over-filtering when municipal data is incomplete or too general.
...
def calculate_parent_fit_score(parent_row, garden_row):
    score = 0.0
    reasons = []
...
def calculate_garden_priority_score(parent_row, garden_row, parent_fit_score):
    ...
    if age_reason == "age_match":
        score += 100
        reasons.append("age_match")

    sibling_status = normalize_text(parent_row.get("sibling_verification_status", ""))
    sibling_garden_id = clean_value(parent_row.get("sibling_garden_id", ""))
    garden_id = clean_value(garden_row.get("garden_id", ""))

    if sibling_status == "approved" and sibling_garden_id == garden_id:
        score += 1000
        reasons.append("approved_sibling_priority")

    if distance_info["same_neighborhood"]:
        score += 50
        reasons.append("neighborhood_priority")

    distance = distance_info["distance_km"]
    preferred_distance = distance_info["preferred_distance_km"]
    if distance is not None and preferred_distance is not None and distance <= preferred_distance:
        score += 35
        reasons.append("within_preferred_distance_priority")

    manual_rank = get_manual_preference_rank(parent_row, garden_id)

    if manual_rank is not None:
        score += {1: 45, 2: 30, 3: 15}.get(manual_rank, 0)
        reasons.append(f"parent_ranked_garden_{manual_rank}")

    score += parent_fit_score * 0.10
    reasons.append("fit_score_tie_breaker")
...
def get_candidate_gardens_for_parent(parent_row, gardens_records, all_garden_ids=None):
    ...
    return candidate_rows, filtering_summary
...
def build_preferences(parents_df, gardens_df):
    ...
    for parent_row in parents_records:
        ...
        candidate_garden_rows, filtering_summary = get_candidate_gardens_for_parent(parent_row, gardens_records, all_garden_ids)
        ...
        for garden_row in candidate_garden_rows:
            ...
            fit_score, reasons, acceptable = calculate_parent_fit_score(parent_row, garden_row)
...
def create_outputs(
    ...
    social_pairing_check_df = build_social_pairing_check(parents_df, parent_match)
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
        ...
        final_df.to_excel(writer, sheet_name="Final_Matching", index=False)
        assignment_df.to_excel(writer, sheet_name="Garden_Assignments", index=False)
        stability_summary_df.to_excel(writer, sheet_name="Stability_Check", index=False)
        blocking_pairs_df.to_excel(writer, sheet_name="Blocking_Pairs", index=False)
        complexity_df.to_excel(writer, sheet_name="Complexity", index=False)
        capacity_df.to_excel(writer, sheet_name="Capacity_Utilization", index=False)
        preference_satisfaction_df.to_excel(writer, sheet_name="Preference_Satisfaction", index=False)
        social_pairing_check_df.to_excel(writer, sheet_name="Social_Pairing_Check", index=False)
        demand_by_criteria_df.to_excel(writer, sheet_name="Demand_By_Criteria", index=False)
        garden_demand_ranking_df.to_excel(writer, sheet_name="Garden_Demand_Ranking", index=False)
        birth_month_audit_df.to_excel(writer, sheet_name="Birth_Month_Audit", index=False)
        social_bonus_df.to_excel(writer, sheet_name="Social_Bonus", index=False)
        mandatory_assignment_df.to_excel(writer, sheet_name="Mandatory_Assignment", index=False)
        planning_review_df.to_excel(writer, sheet_name="Planning_Review", index=False)
        manual_exception_log_df.to_excel(writer, sheet_name="Manual_Exception_Log", index=False)
        distance_audit_df.to_excel(writer, sheet_name="Distance_Audit", index=False)

        if DEBUG_OUTPUT:
            proposal_log_df.to_excel(writer, sheet_name="Proposal_Log", index=False)
            candidate_filtering_df.to_excel(writer, sheet_name="Candidate_Filtering", index=False)
            parent_preferences_df.to_excel(writer, sheet_name="Parent_Preferences", index=False)
            garden_rankings_df.to_excel(writer, sheet_name="Garden_Rankings", index=False)
